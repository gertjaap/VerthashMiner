...
static const char *logPrioNames[] = {
    "ERROR",
    "WARN",
    "INFO",
    "DEBUG"
...
void applog(ELogPriority prio, const char *fmt, ...)
{
    ...
    int logNameIndex = (int)prio;
    if (logNameIndex > 3) { logNameIndex = 0; }
...
static size_t all_data_cb(const void *ptr, size_t size, size_t nmemb, void *user_data)
{
    ...
    newmem = (char*)realloc(db->buf, newlen + 1);
    if (!newmem)
    {
        return 0;
    }
...
static size_t resp_hdr_cb(void *ptr, size_t size, size_t nmemb, void *user_data)
{
    ...
    tmp = (char*)memchr((char*)ptr, ':', ptrlen);
    if (!tmp || (tmp == ((char*)ptr)) ) // skip empty keys / blanks
    {
        goto out;
    }
    slen = tmp - ((char*)ptr);
...
json_t *json_rpc_call(CURL *curl, const char *url,
                      const char *user, const char *pass, const char *rpc_req,
                      int *curl_err, int flags)
{
    ...
    long timeout = (flags & JSON_RPC_LONGPOLL) ? opt_longpoll_timeout : 30;
    ...
    if (rc) {
        ...
        if ((rc == CURLE_ABORTED_BY_CALLBACK) && (abort_flag == true))
        {
...
bool stratum_send_line(struct stratum_ctx *sctx, char *s)
{
    ...
    mtx_lock(&sctx->sock_lock);
    ret = send_line(sctx, s);
    mtx_unlock(&sctx->sock_lock);

    return ret;
}

static bool socket_full(curl_socket_t sock, curl_socket_t dummy, int timeout)
{
    struct timeval tv;
    fd_set rd;
    ...
    int rc = select(sock + 1, &rd, NULL, NULL, &tv);

    if(FD_ISSET(dummy, &rd) > 0)
    {
...
{
    return strlen(sctx->sockbuf) || socket_full(sctx->sock, sctx->dummy_socket, timeout);
}
...
static void stratum_buffer_append(struct stratum_ctx *sctx, const char *s)
{
    ...
    vold = strlen(sctx->sockbuf);
    vnew = vold + strlen(s) + 1;
    if (vnew >= sctx->sockbuf_size) {
        sctx->sockbuf_size = vnew + (RBUFSIZE - (vnew % RBUFSIZE));
        sctx->sockbuf = (char*)realloc(sctx->sockbuf, sctx->sockbuf_size);
    }
    strcpy(sctx->sockbuf + vold, s);
}
...
char *stratum_recv_line(struct stratum_ctx *sctx)
{
    ...
    if (!strstr(sctx->sockbuf, "\n"))
    {
        ...
        time(&rstart);
        if (!socket_full(sctx->sock, sctx->dummy_socket, 60))
        {
        ...
        do {
            ...
            if (rc != CURLE_OK) {
                if (rc != CURLE_AGAIN || !socket_full(sctx->sock, sctx->dummy_socket, 1)) {
#else
            ...
            if (n < 0) {
                if (!socket_blocks() || !socket_full(sctx->sock, sctx->dummy_socket, 1)) {
#endif
                    ret = false;
                    break;
                }
        ...
        // double check for abort_flag to avoid possible infinity loop(first is after stratum_socket_full in stratum thread.
...
start:
    ...
    if (!socket_full(sctx->sock, sctx->dummy_socket, 30))
    {
    ...
    xn2_size = (int)json_integer_value(json_array_get(res_val, 2));
    if (!xn2_size) {
...
static bool stratum_reconnect(struct stratum_ctx *sctx, json_t *params)
{
    ...
    applog(LOG_INFO, "Server requested reconnection to %s", url);
...
static bool stratum_show_message(struct stratum_ctx *sctx, json_t *id, json_t *params)
{
    ...
    if (val)
        applog(LOG_INFO, "MESSAGE FROM SERVER: %s", json_string_value(val));
